...
def R_from_fc_files(final_coords):
    ...
    return R_sparse, count

#@njit(parallel=True) #count is not updated fast enough
def find_relations(final_coords):
    ...
    return row_indices[:count], col_indices[:count], values[:count], N, count

def R_from_fc_sparse(final_coords):
    row_indices, col_indices, values, N, count = find_relations(final_coords)
    R_sparse = csr_matrix((values, (row_indices, col_indices)), shape=(N, N)) #the problem seems to be here
    return R_sparse, count
