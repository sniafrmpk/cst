...
def LMCs_v3(L, k_max, i, j):
    ...
    # The idea is to move down from j and at each level (one link below j in the LMCs) 
    # find all the elements that have a k_max-1 length chain between them and i (the start of
    # all the chains). This can be done by going to the k_max-1th power of L's ith row. All non-zero
    # columns have indices that 
    for l in range(1,k_max): # slightly confused if it should be k_max + 1?
        ...
        Ltopower = Ld[next_level-1]
        row_start = Ltopower.indptr[i]
        row_end = Ltopower.indptr[i+1]
        columns = Ltopower.indices[row_start:row_end]
        for candidate in columns:
            isrelated = False
...
def LMCs(N = 10000, D = 4, l = 0.2, i = 0, j = 9999):
    t00 = time.time()
    fc = Minkowski_cube(N, D, l)
    R = R_from_fc(fc)
    k_max, R_sparse, R_2 = causet_tau_c(R, i, j)
    L = L_from_R(R_sparse, R_2)
    all_levels = LMCs_v3(L, k_max, i ,j)
    
    tf = time.time()
    print(f"---Total time to find LMCs for N = {N}, D = {D}, l = {l}, i = {i} and j = {j}: {tf - t00: .2f} s---")
    return fc, R, L, all_levels
